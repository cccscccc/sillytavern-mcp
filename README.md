...
- `st_import_character` 导入成功后返回规范的 `char:xxx.png` handle（以前只给文件名主干，既没有 `char:` 前缀也没有扩展名），并附上「先体检、再 X 光」的下一步提示。返回的 handle 可以直接喂给后续工具，不用自己拼。
- 扩展名不是硬拼的：导入后回查一次拿真实 avatar，因为 `.png` / `.json` 等格式存进酒馆后的名字不一样。
...
- 报告口径拆开：系统块 / 按深度插入 / 对话历史 / 合计，都是 token 估算，方便判断提示词规模。
